...
from RoundingUtility import Round
...
class FXfamily(object):
    ...
    def __init__(self, n_bits=64, n_intbits=None, round=Round.round_trunc):
...
class FXnum(object):
    ...
    # Rounding method supported
    def __mul__(self, other):
        """ Multiply with rounding """
        if self.family.round is None:
            raise TypeError("Round function cannot be None type")
        other = self._CastOrFail_(other)
        return FXnum._rawbuild(self.family, self.family.round(self.scaledval*other.scaledval, self.family.fraction_bits))
    ...
    def div2round(self, shift):
        return FXnum._rawbuild(self.family, self.family.round(self.scaledval, shift))
    """
    def __rshift__(self, shift):
        return self.div2round(shift)
    """
    ...
    def __truediv__(self, other):
         ...
         other = self._CastOrFail_(other)
         return FXnum._rawbuild(self.family, (self.scaledval * self.family.scale) // other.scaledval)
